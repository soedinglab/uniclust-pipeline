...
function make_finalize() {
    ...
    local BOOSTDB="${BASE}/${BOOSTPREFIX}_${RELEASE}"
    local TMPDIR="$5"

    cp -f "${BOOSTDB}_cs219_binary.ffdata" "${DB}_cs219.ffdata"
	cp -f "${BOOSTDB}_cs219_binary.ffindex" "${DB}_cs219.ffindex"
    cp -f "${BOOSTDB}_cs219_plain.ffdata" "${DB}_cs219_plain.ffdata"
	cp -f "${BOOSTDB}_cs219_plain.ffindex" "${DB}_cs219_plain.ffindex"
    ...
    rm -f "${DB}_cs219_plain.ffdata""${DB}_cs219_plain.ffindex"
